...
public class DepthLimited {
	...
	public static List<Node> nextStates(Node currentNode){
		...
		//takes list of new boards and checks which swaps are legal by checking limits of board
		...
		.filter(x->x.x >= 0 && x.x <= 2 && x.y >= 0 && x.y <= 2)
		.map(pair -> {
	...
	public static boolean isVisited(Node node1, int[][] child) {
		 Node node = node1;
		while(node != null) {
			if (Arrays.deepEquals(node.matrix, child)) return true;
			node = node.parent;
		}
		return false;
	}
	...
	//finds path to solution 
	//depth limit is set to 10
	public static Node findPath(Node node) {
		int[][] currentNode = node.matrix;
		if (goalTest(currentNode)) return node;
		else if (node.level >= 10) return null;
		else {
			List<Node> children= nextStates(node);
			for(Node child: children) {
				Node resultTry = findPath(child);
				if (resultTry != null) {
					return resultTry;
				}
			}
