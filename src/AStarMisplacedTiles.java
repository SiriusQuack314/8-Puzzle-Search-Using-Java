...
public class AStarMisplacedTiles
{
	...
	public AStarMisplacedTiles(Node root)
	{
		queue.add(root);
	...
	public static void expandNode(Node node)
	{
		...
		for (int r = 0; r < options.length; r++)
		{
			if (!(options[r][0] < 0 || options[r][1] < 0 || options[r][0] > 2 || options[r][1] > 2))
			{
				...
				child.cost = numberOfMisplacedTiles(child);
	...
	public static int numberOfMisplacedTiles(Node node)
	{
		int sum = 0;
